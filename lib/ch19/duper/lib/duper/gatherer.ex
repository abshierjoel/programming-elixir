defmodule Duper.Gatherer do
  ...
  def start_link(worker_count),
    do: GenServer.start_link(__MODULE__, worker_count, name: __MODULE__)

  def done(), do: GenServer.cast(__MODULE__, :done)
  def result(path, hash), do: GenServer.cast(__MODULE__, {:result, path, hash})
